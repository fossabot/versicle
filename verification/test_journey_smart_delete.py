...
def test_smart_delete_journey(page: Page, demo_epub_path):
    ...
    # Ensure app is ready by checking for empty state text
    expect(page.get_by_text("Your library is empty")).to_be_visible(timeout=10000)

    # 1. Import Book
    print("Importing book...")
    # Make sure input is attached
    file_input = page.locator("data-testid=hidden-file-input")
    expect(file_input).to_be_attached(timeout=5000)
    file_input.set_input_files(demo_epub_path)
    ...
    # Confirm Offload (Dialog)
    # The new implementation adds a confirmation step
    print("Confirming offload...")
    expect(page.get_by_text("Offload Book")).to_be_visible()
    ...
    expect(restore_input).to_be_attached()
    ...
    expect(page.get_by_test_id("offloaded-overlay")).not_to_be_visible(timeout=10000)
